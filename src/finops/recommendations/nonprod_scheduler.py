...
def _get_hourly_cpu_max(cw_client: Any, instance_id: str) -> list[float]:
    ...
    Returns a list of max CPU values (one per hour). May be shorter than expected
    if the instance was not running for the full window.
    """
    ...
    try:
        resp = cw_client.get_metric_statistics(
            ...
            StartTime=start,
            EndTime=end,
            Period=3600,          # one data point per hour
            Statistics=["Maximum"],
        )
            ...
            for page in pag.paginate(
                ...
                for reservation in page["Reservations"]:
                    for inst in reservation["Instances"]:
                        ...
                        cpu_samples = _get_hourly_cpu_max(cw, iid)
