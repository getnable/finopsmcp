...
async def get_rightsizing_recommendations(
    ...
    try:
        ...
        savings_ctx = await asyncio.to_thread(detect_savings_context)
...
# ── Extracted per-family tool modules ────────────────────────────────────────
# Imported here, after mcp and every shared helper above are defined, so each
# module's @mcp.tool() decorators register against the same telemetry-wrapped mcp.
# The names are re-exported so `finops.server.<tool>` keeps resolving for internal
# callers and tests: extraction moves where a tool lives, not its public address.
from .tools import recommendations as _tools_recommendations  # noqa: E402,F401
